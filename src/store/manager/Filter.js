...
export default class Filter {
    constructor(name, options) {
        ...
        if (typeof options !== 'object')
            throw Error(
                'Failed to construct Filter. Options map is not provided',
            );
    ...
    get currentManager() {
        return this.store?.getters
            ? this.store.getters[this.current] || {}
            : {};
    }
    ...
    get error() {
        return typeof this.currentManager.error === 'boolean'
            ? this.currentManager.error
            : false;
    }

    get loading() {
        return typeof this.currentManager.loading === 'boolean'
            ? this.currentManager.loading
            : true;
    }

    get canFetchPrevious() {
        return typeof this.currentManager.canFetchPrevious === 'boolean'
            ? this.currentManager.canFetchPrevious
            : false;
    }

    get canFetchNext() {
        return typeof this.currentManager.canFetchNext === 'boolean'
            ? this.currentManager.canFetchNext
            : false;
    }

    get index() {
        return typeof this.currentManager.index === 'number'
            ? this.currentManager.index
            : 0;
    }
    ...
    fetchNext() {
        ...
        else console.error('Cannot call "fetchNext" method of', this.current);
    }
    ...
    fetchPrevious() {
        if (typeof this.currentManager.fetchPrevious === 'function')
            ...
            console.error(
                'Cannot call "fetchPrevious" method of',
                this.current,
            );
    }
    ...
    changeFilterValue(option) {
        ...
        if (typeof this.currentManager.initialFetch === 'function')
            ...
            console.error(
                'Failed to fetch selected filter option. "initialFetch" is not a function',
            );
