...
export default class Timeline {
    constructor(
        ...
        if (typeof initialFuntion !== 'function')
            throw Error(
                'Cannot create timeline. Initial function is not provided',
            );
        if (typeof fetchFunction !== 'function')
            throw Error(
                'Cannot create timeline. Fetch function is not provided',
            );
        if (keyName === void 0)
    ...
    async fetch() {
        ...
        try {
            ...
            if (this.data?.length) {
                ...
                this.next = await this.fetchFunction(
                    key,
                    this.pageSize,
                    this.store,
                );
    ...
    addLatestItem(item, keyName = this.keyName) {
        if (this.isLive) {
            if (this.data?.length && this.data[0][keyName] === item[keyName])
                console.error(
                    'internal error: attempted to add duplicate item to timeline.',
                );
            else {
