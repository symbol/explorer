...
const managers = [
    ...
    new Pagination({
        ...
        fetchFunction: (pageInfo, filterValue, store) =>
            RestrictionService.getMosaicRestrictionList(
                pageInfo,
                filterValue,
                store.getters.getCurrentMosaicId,
            ),
        pageInfo: {
            pageSize: Constants.PageSize,
        },
    ...
    new DataSet('info', (hexOrNamespace) =>
        MosaicService.getMosaicInfo(hexOrNamespace),
    ),
    new Pagination({
        name: 'metadatas',
        fetchFunction: (pageInfo, filterValue, store) =>
            MosaicService.getMosaicMetadataList(
                pageInfo,
                filterValue,
                store.getters.getCurrentMosaicId,
            ),
    ...
    actions: {
        ...
        fetchMosaicInfo(context, payload) {
            ...
            context.getters.info
                .setStore(context)
                .initialFetch(payload.mosaicId);
            context.getters.restrictions
                .setStore(context)
                .initialFetch(payload.mosaicId);
