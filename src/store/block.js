...
const managers = [
    ...
    new Pagination({
        ...
        fetchFunction: (pageInfo, filterValue, store) =>
            BlockService.getBlockTransactionList(
                pageInfo,
                filterValue,
                store.getters.currentBlockHeight,
            ),
        pageInfo: {
            pageSize: 10,
        },
    ...
    new DataSet('blockReceipts', (height) =>
        BlockService.getBlockReceiptsInfo(height),
    ),
    new DataSet('info', (height) => BlockService.getBlockInfo(height)),
];
...
export default {
    ...
    getters: {
        ...
        getRecentList: (state) =>
            state.timeline?.data?.filter((item, index) => index < 4) || [],
        getSubscription: (state) => state.subscription,
        blockInfo: (state) => state.info?.data?.blockInfo || {},
        merkleInfo: (state) => state.info?.data?.merkleInfo || {},
        inflationReceipt: (state) =>
            state.blockReceipts?.data?.transactionReceipt?.inflationReceipt ||
            [],
        balanceTransferReceipt: (state) =>
            state.blockReceipts?.data?.transactionReceipt
                ?.balanceTransferReceipt || [],
        balanceChangeReceipt: (state) =>
            state.blockReceipts?.data?.transactionReceipt
                ?.balanceChangeReceipt || [],
        artifactExpiryReceipt: (state) =>
            state.blockReceipts?.data?.transactionReceipt
                ?.artifactExpiryReceipt || [],
        resolutionStatement: (state) =>
            state.blockReceipts?.data?.resolutionStatements || [],
        currentBlockHeight: (state) => state.currentBlockHeight,
        infoText: (s, g, rs, rootGetters) =>
            rootGetters['ui/getNameByKey']('chainHeight') +
            ': ' +
            (rootGetters['chain/getChainInfo'] &&
            rootGetters['chain/getChainInfo'].currentHeight
                ? rootGetters['chain/getChainInfo'].currentHeight
                : 0),
    },
    mutations: {
        ...
        currentBlockHeight: (state, currentBlockHeight) =>
            Vue.set(state, 'currentBlockHeight', currentBlockHeight),
    },
    actions: {
        ...
        async subscribe({ commit, dispatch, getters, rootGetters }) {
            if (getters.getSubscription === null) {
                const subscription = await ListenerService.subscribeNewBlock(
                    async (item) => {
                        const latestBlock = await BlockService.getBlockByHeight(
                            item.height.compact(),
                        );
                        ...
                        getters.timeline.addLatestItem(
                            {
                                ...latestBlock,
                                date: helper.convertToUTCDate(
                                    latestBlock.timestamp,
                                ),
                                age: helper.convertToUTCDate(
                                    latestBlock.timestamp,
                                ),
        ...
        fetchBlockInfo: (context, payload) => {
            ...
            context.getters.blockReceipts
                .setStore(context)
                .initialFetch(payload.height);
            context.getters.blockTransactions
                .setStore(context)
                .initialFetch(payload.height);
        },
