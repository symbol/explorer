...
const managers = [
    new Pagination({
        ...
        fetchFunction: (pageInfo, filterVaule) =>
            AccountService.getAccountList(pageInfo, filterVaule),
        pageInfo: {
    ...
    new Pagination({
        ...
        fetchFunction: (pageInfo, filterValue, store) =>
            AccountService.getAccountHarvestedBlockList(
                pageInfo,
                store.getters.getCurrentAccountAddress,
            ),
        pageInfo: {
    ...
    actions: {
        ...
        async fetchAccountDetail(context, payload) {
            ...
            context.getters.info
                .setStore(context)
                .initialFetch(payload.address);
            context.getters.OwnedMosaic.setStore(context).initialFetch(
                payload.address,
            );
            context.getters.OwnedNamespace.setStore(context).initialFetch(
                payload.address,
            );
            context.getters.multisig
                .setStore(context)
                .initialFetch(payload.address);
            context.getters.transactions
                .setStore(context)
                .initialFetch(payload.address);
            ...
            context.getters.mosaicAddressRestrictions
                .setStore(context)
                .initialFetch(payload.address);
            context.getters.harvestedBlocks
                .setStore(context)
                .initialFetch(payload.address);
            context.getters.accountRestrictions
                .setStore(context)
                .initialFetch(payload.address);
            context.getters.hashLocks
                .setStore(context)
                .initialFetch(payload.address);
            context.getters.secretLocks
                .setStore(context)
                .initialFetch(payload.address);
        },
