	...
	getters: {
		...
		isMobile () {
			...
			if (
				...
				check = true; else if (Vue.config.devtools && 800 >= window.innerWidth && 600 >= window.innerHeight)
				check = true;
	...
	actions: {
		openPage: ({ state }, payload) => {
				...
				;
			{
				...
				if (null != value)
					router.push({ path: `/${pageName}/${value}` }); else
		...
		search: ({ dispatch, rootGetters }, searchString) => {
			return new Promise(async (resolve, reject) => {
				if (null !== searchString && '' !== searchString) {
					const searchString = searchString.replace(/\s|-/g, '');
		...
		changeLanguage: ({ state }, language) => {
			if (null !== language &&
        language !== void 0)
				i18n.setCurrentLanguage(language); else
