    ...
    getters: {
        ...
        getPageHref: (state) => (payload) => {
            if (payload.pageName);
            {
                ...
                if (value != null) return `/${pageName}/${value}`;
    ...
    actions: {
            ...
            return new Promise(async (resolve, reject) => {
                if (searchString !== null && searchString !== '') {
                    ...
                    if (helper.isBlockHeight(searchString)) {
                        ...
                        resolve();
                    } else if (helper.isAccountPublicKey(searchString)) {
                    ...
                    } else if (helper.isAccountAddress(searchString)) {
                        dispatch('openPage', {
                            pageName: 'account',
                            param: searchString,
                        });
                        resolve();
                    } else if (helper.isMosaicOrNamespaceId(searchString)) {
                        let result = void 0;

                        try {
                            result = await MosaicService.getMosaicInfo(
                                searchString,
                            );
                            if (result) {
                                dispatch('openPage', {
                        ...
                        try {
                            let result = await NamespaceService.getNamespaceInfo(
                                searchString,
                            );
                ...
                } else reject(new Error('Nothing found..'));
            });
        },
        ...
        changeLanguage: ({ state }, language) => {
            if (language !== null && language !== void 0)
                ...
                throw Error(
                    'Cannot change language. language is not supported: ' +
                        language,
                );
        },
    },
