...
class AnnounceService {
        ...
        return new Promise((resolve, reject) => {
            ...
            const customWsEndpoint = `${
                localStorage.getItem('currentNode') ||
                    globalConfig.peersApi.defaultNode |> helper.httpToWsUrl
            }/ws`;
    ...
    static getSignedHashLosck = (signedTransaction, account) => {
        const networkCurrencyMosaicId = new MosaicId(
            http.networkCurrency.mosaicId,
        );
        ...
        const hashLockTransaction = HashLockTransaction.create(
            ...
            new Mosaic(
                ...
                UInt64.fromUint(
                    10 * Math.pow(10, http.networkCurrency.divisibility),
                ),
            ),
            UInt64.fromUint(480),
            signedTransaction,
    ...
    static multisigAccountModification = async ({
        ...
        const addressAdditions = additions.map((addition) => {
            if (typeof addition === 'string' && addition.length === 64)
                return PublicAccount.createFromPublicKey(addition, networkType)
                    .address;
        ...
        const addressDeletions = deletions.map((delition) => {
            if (typeof delition === 'string' && delition.length === 64)
                return PublicAccount.createFromPublicKey(delition, networkType)
                    .address;
