...
class TransactionService {
    ...
    static getTransaction = async (hash, transactionGroup) => {
        const transaction = await http.createRepositoryFactory
    ...
    static searchTransactions = async (transactionSearchCriteria) => {
        const searchTransactions = await http.createRepositoryFactory
            .createTransactionRepository()
    ...
    static getTransactionEffectiveFee = async (hash) => {
        let effectiveFee = await http.createRepositoryFactory
            .createTransactionRepository()
     ...
     */
    static getTransactionInfo = async (
        hash,
        ...
        const [
            ...
            BlockService.getBlockInfo(
                UInt64.fromUint(transaction.transactionInfo.height),
            ),
            this.getTransactionEffectiveFee(hash),
            this.getTransactionStatus(hash),
            BlockService.getMerkleTransaction(
                UInt64.fromUint(transaction.transactionInfo.height),
                transaction.transactionInfo.merkleComponentHash,
            ),
        ]);

        const formattedTransaction = await this.createTransactionFromSDK(
            transaction,
        );
    ...
    static getTransactionList = async (pageInfo, filterVaule) => {
        ...
        const searchTransactions = await this.searchTransactions(
            searchCriteria,
        );
        ...
        await Promise.all(
            transactions.data.map(async (transaction) => {
                if (transaction?.recipientAddress)
                    return (transaction.transactionBody.recipient = await helper.resolvedAddress(
    ...
    static formatTransaction = (transaction) => ({
        ...
        transactionInfo: this.formatTransactionInfo(
    ...
    static formatTransactionBody = (transactionBody) => {
        switch (transactionBody.type) {
            ...
            case TransactionType.NAMESPACE_REGISTRATION:
                let parentIdHex = transactionBody.parentId
                    ? transactionBody.parentId.toHex()
                    : '';

                let duration = transactionBody.duration
                    ? transactionBody.duration.compact()
                    : 0;

                return {
                    transactionType: transactionBody.type,
                    recipient:
                        http.networkConfig.NamespaceRentalFeeSinkAddress
                            .address,
                    registrationType:
                        Constants.NamespaceRegistrationType[
                            transactionBody.registrationType
                        ],
                    ...
                    parentId:
                        parentIdHex === ''
                            ...
                            : parentIdHex,
                    duration:
                        duration === 0 ? Constants.Message.UNLIMITED : duration,
                };
            ...
            case TransactionType.ADDRESS_ALIAS:
                return {
                    transactionType: transactionBody.type,
                    aliasAction:
                        Constants.AliasAction[transactionBody.aliasAction],
                    namespaceId: transactionBody.namespaceId.toHex(),
            ...
            case TransactionType.MOSAIC_DEFINITION:
                return {
                    transactionType: transactionBody.type,
                    recipient:
                        http.networkConfig.MosaicRentalSinkAddress.address,
            ...
            case TransactionType.MOSAIC_SUPPLY_CHANGE:
                return {
                    ...
                    mosaicId: transactionBody.mosaicId.id.toHex(),
                    action:
                        Constants.MosaicSupplyChangeAction[
                            transactionBody.action
                        ],
            ...
            case TransactionType.MULTISIG_ACCOUNT_MODIFICATION:
                return {
                    ...
                    addressAdditions: transactionBody.addressAdditions.map(
                        (address) => address.address,
                    ),
                    addressDeletions: transactionBody.addressDeletions.map(
                        (address) => address.address,
                    ),
                };
            ...
            case TransactionType.AGGREGATE_COMPLETE:
                return {
                    transactionType: transactionBody.type,
                    innerTransactions: transactionBody.innerTransactions.map(
                        (transaction) => this.formatTransaction(transaction),
                    ),
                    cosignatures: transactionBody.cosignatures.map(
                        (cosigner) => ({
                            ...cosigner,
            ...
            case TransactionType.HASH_LOCK:
                return {
                    ...
                    amount: helper.toNetworkCurrency(
                        transactionBody.mosaic.amount,
                    ),
            ...
            case TransactionType.SECRET_LOCK:
                return {
                    ...
                    recipient: transactionBody.recipientAddress,
                    hashAlgorithm:
                        Constants.LockHashAlgorithm[
                            transactionBody.hashAlgorithm
                        ],
                };
            ...
            case TransactionType.ACCOUNT_ADDRESS_RESTRICTION:
                return {
                    ...
                    restrictionAddressAdditions: transactionBody.restrictionAdditions.map(
                        (restriction) => {
                    ...
                    restrictionAddressDeletions: transactionBody.restrictionDeletions.map(
                        (restriction) => {
                            if (restriction instanceof Address)
                                return restriction.address;
            ...
            case TransactionType.ACCOUNT_MOSAIC_RESTRICTION:
                return {
                    ...
                    restrictionMosaicAdditions: transactionBody.restrictionAdditions.map(
                        (mosaic) => mosaic.id.toHex(),
                    ),
                    restrictionMosaicDeletions: transactionBody.restrictionDeletions.map(
                        (mosaic) => mosaic.id.toHex(),
                    ),
                };
            ...
            case TransactionType.ACCOUNT_OPERATION_RESTRICTION:
                return {
                    ...
                    restrictionOperationAdditions: transactionBody.restrictionAdditions.map(
                        (operation) => operation,
                    ),
                    restrictionOperationDeletions: transactionBody.restrictionDeletions.map(
                        (operation) => operation,
                    ),
                };
                ...
                return {
                    ...
                    previousRestrictionType:
                        Constants.MosaicRestrictionType[
                            transactionBody.previousRestrictionType
                        ],
                    previousRestrictionValue: transactionBody.previousRestrictionValue.compact(),
                    newRestrictionType:
                        Constants.MosaicRestrictionType[
                            transactionBody.newRestrictionType
                        ],
    ...
    static createStandaloneTransactionFromSDK = async (transactionDTO) => {
        const transactionObj = {
            ...
            deadline: helper.convertDeadlinetoDate(
                transactionDTO.deadline.value,
            ),
        ...
        switch (transactionDTO.type) {
            ...
            case TransactionType.MULTISIG_ACCOUNT_MODIFICATION:
                return CreateTransaction.multisigAccountModification(
                    transactionObj,
                );
            case TransactionType.HASH_LOCK:
            ...
            case TransactionType.ACCOUNT_ADDRESS_RESTRICTION:
                return CreateTransaction.accountAddressRestriction(
                    transactionObj,
                );
            case TransactionType.ACCOUNT_MOSAIC_RESTRICTION:
                return CreateTransaction.accountMosaicRestriction(
                    transactionObj,
                );
            case TransactionType.ACCOUNT_OPERATION_RESTRICTION:
                return CreateTransaction.accountOperationRestriction(
                    transactionObj,
                );
            case TransactionType.MOSAIC_ADDRESS_RESTRICTION:
                return CreateTransaction.mosaicAddressRestriction(
                    transactionObj,
                );
            case TransactionType.MOSAIC_GLOBAL_RESTRICTION:
                return CreateTransaction.mosaicGlobalRestriction(
                    transactionObj,
                );
            case TransactionType.ACCOUNT_METADATA:
            ...
            default:
                throw new Error(
                    'Unimplemented transaction with type ' +
                        transactionDTO.type,
                );
        }
    };
    ...
    static createTransactionFromSDK = async (transactionDTO) => {
        if (
            transactionDTO.type === TransactionType.AGGREGATE_BONDED ||
            transactionDTO.type === TransactionType.AGGREGATE_COMPLETE
        ) {
            const innerTransactions = transactionDTO.innerTransactions
                ? await Promise.all(
                      transactionDTO.innerTransactions.map(
                          async (transaction, index) => {
                              return {
                                  index: index + 1,
                                  ...(await this.createStandaloneTransactionFromSDK(
                                      transaction,
                                  )),
                              };
                          },
            ...
            return {
                ...transactionDTO,
                innerTransactions,
                cosignatures: transactionDTO.cosignatures
                    ? transactionDTO.cosignatures.map((cosignature) => {
                          return {
                              ...cosignature,
