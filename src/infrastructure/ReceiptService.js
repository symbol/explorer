...
import { ReceiptExtractor } from '../infrastructure';
...
class ReceiptService {
	...
	static searchReceipts = async transactionStatementSearchCriteria => {
		const searchReceipts = await http.createRepositoryFactory
	 ...
	 */
	static searchAddressResolutionStatements =
		async resolutionStatementSearchCriteria => {
			const searchAddressResolutionStatements =
				await http.createRepositoryFactory
					.createReceiptRepository()
	 ...
	 */
	static searchMosaicResolutionStatements =
		async resolutionStatementSearchCriteria => {
			const searchMosaicResolutionStatements =
				await http.createRepositoryFactory
					.createReceiptRepository()
	...
	static streamerReceipts = async searchCriteria => {
		const streamerReceipts = await http
			.transactionStatementPaginationStreamer()
	...
	static streamerAddressResolution = async searchCriteria => {
		const streamerAddressResolution = await http
			.addressResolutionStatementPaginationStreamer()
			.search(searchCriteria)
			.pipe(take(10), toArray())
			.toPromise();
	...
	static streamerMosaicResolution = async searchCriteria => {
		const streamerMosaicResolution = await http
			.mosaicResolutionStatementPaginationStreamer()
			.search(searchCriteria)
			.pipe(take(10), toArray())
			.toPromise();
	...
	static createReceiptTransactionStatement = async transactionStatement => {
		...
		default:
			throw new Error('Unimplemented receipt transaction statement with type ' +
	...
	static transactionStatementBuilder(transactionStatement) {
		...
		return {
			balanceChangeStatement: {
				receiptTransactionStatementType:
					Constants.ReceiptTransactionStatementType.BalanceChangeReceipt,
				data: balanceChangeReceipt
			},
			balanceTransferStatement: {
				receiptTransactionStatementType:
					Constants.ReceiptTransactionStatementType.BalanceTransferReceipt,
				data: balanceTransferReceipt
			},
			inflationStatement: {
				receiptTransactionStatementType:
					Constants.ReceiptTransactionStatementType.InflationReceipt,
				data: inflationReceipt
			},
			artifactExpiryStatement: {
				receiptTransactionStatementType:
					Constants.ReceiptTransactionStatementType.ArtifactExpiryReceipt,
