...
class ReceiptService {
     ...
     */
    static searchAddressResolutionStatements = async (
        resolutionStatementSearchCriteria,
    ) => {
        const searchAddressResolutionStatements = await http.createRepositoryFactory
            .createReceiptRepository()
        ...
        return {
            ...searchAddressResolutionStatements,
            data: this.formatResolutionStatement(
     ...
     */
    static searchMosaicResolutionStatements = async (
        resolutionStatementSearchCriteria,
    ) => {
        const searchMosaicResolutionStatements = await http.createRepositoryFactory
            .createReceiptRepository()
        ...
        return {
            ...
            data: this.formatResolutionStatement(
                searchMosaicResolutionStatements.data,
            ),
        };
    };
        ...
        return resolutionStatement.map((statement) => {
            if (statement.resolutionType === ResolutionType.Address) {
                return {
                    ...
                    resolutionType:
                        Constants.ResolutionType[statement.resolutionType],
                    unresolved: statement.unresolved.toHex(),
                    addressResolutionEntries: statement.resolutionEntries.map(
                        (resolutionEntry) => resolutionEntry.resolved.address,
                    ),
                };
            }
            ...
            if (statement.resolutionType === ResolutionType.Mosaic) {
                return {
                    ...
                    resolutionType:
                        Constants.ResolutionType[statement.resolutionType],
                    unresolved: statement.unresolved.toHex(),
                    mosaicResolutionEntries: statement.resolutionEntries.map(
                        (resolutionEntry) => resolutionEntry.resolved.toHex(),
                    ),
                };
            }
        });
