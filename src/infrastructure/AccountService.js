...
class AccountService {
    ...
    static getAccounts = async (addresses) => {
        const accounts = await http.createRepositoryFactory
            .createAccountRepository()
            .getAccountsInfo(
                addresses.map((a) => Address.createFromRawAddress(a)),
            )
            .toPromise();
    ...
    static searchAccounts = async (accountSearchCriteria) => {
        ...
        return {
            ...searchAccounts,
            data: searchAccounts.data.map((account) =>
    ...
    static getAccountList = async (pageInfo, filterVaule) => {
        ...
        return {
            ...
            data: accountInfos.data.map((account) => ({
                ...
                lastActivity: helper.getLastActivityHeight(
                    account.activityBucket,
                ),
                accountAliasName: this.extractAccountNamespace(
                    account,
                    accountNames,
                ),
            })),
    ...
    static getAccountInfo = async (address) => {
        ...
        const accountNames = await NamespaceService.getAccountsNames([
        ...
        return {
            ...
            supplementalPublicKeys: {
                ...
                voting: Array.isArray(accountInfo.supplementalPublicKeys.voting)
                    ? accountInfo.supplementalPublicKeys.voting.map(
                          (voting) => voting.publicKey,
                      )
                    : accountInfo.supplementalPublicKeys.voting,
            },
            accountAliasName: this.extractAccountNamespace(
                accountInfo,
                accountNames,
     ...
     */
    static getAccountTransactionList = async (
        pageInfo,
        filterVaule,
        address,
    ) => {
        ...
        const searchTransactions = await TransactionService.searchTransactions(
            searchCriteria,
        );
    ...
    static getAccountNamespaceList = async (pageInfo, filterVaule, address) => {
        ...
        const accountNamespaces = await NamespaceService.searchNamespaces(
            searchCriteria,
        );
    ...
    static getAccountHarvestedBlockList = async (pageInfo, address) => {
        ...
        const accountHarvestedBlockList = await BlockService.searchBlocks(
            searchCriteria,
        );
    ...
    static getAccountMetadataList = async (pageInfo, filterVaule, address) => {
        ...
        const accountMetadatas = await MetadataService.searchMetadatas(
            searchCriteria,
        );
    ...
    static getAccountHashLockList = async (pageInfo, address) => {
        ...
        const accountHashLocks = await LockService.searchHashLocks(
            searchCriteria,
        );
    ...
    static getAccountSecretLockList = async (pageInfo, address) => {
        ...
        const accountSecretLocks = await LockService.searchSecretLocks(
            searchCriteria,
        );
    ...
    static formatAccountInfo = (accountInfo) => ({
        ...
        publicKey:
            accountInfo.publicKeyHeight.compact() > 0
        ...
        supplementalPublicKeys: this.formatSupplementalPublicKeys(
            accountInfo.supplementalPublicKeys,
        ),
        importance: helper.ImportanceScoreToPercent(
            accountInfo.importance.compact(),
        ),
    ...
    static formatSupplementalPublicKeys = (supplementalPublicKeys) => ({
        ...supplementalPublicKeys,
        linked:
            supplementalPublicKeys.linked?.publicKey ||
            Constants.Message.UNAVAILABLE,
        node:
            supplementalPublicKeys.node?.publicKey ||
            Constants.Message.UNAVAILABLE,
        vrf:
            supplementalPublicKeys.vrf?.publicKey ||
            Constants.Message.UNAVAILABLE,
    ...
    static extractAccountNamespace = (accountInfo, accountNames) => {
        let accountName = accountNames.find(
            (name) => name.address === accountInfo.address,
        );
        const name =
            accountName.names.length > 0
                ? accountName.names[0].name
                : Constants.Message.UNAVAILABLE;
