...
class CreateTransaction {
    ...
    static addressAlias = async (transactionObj) => {
        ...
        const namespaceName = namespaceNames.find(
            (namespace) =>
                namespace.namespaceId === transactionObj.namespaceId.toHex(),
        );
    ...
    static mosaicDefinition = async (transactionObj) => {
        const resolvedMosaic = await helper.resolveMosaicId(
    ...
    static mosaicSupplyChange = async (transactionObj) => {
        ...
        return {
            ...
            transactionBody: {
                ...
                mosaicId: resolvedMosaic.toHex(),
                action:
                    Constants.MosaicSupplyChangeAction[transactionObj.action],
    ...
    static hashLock = async (transactionObj) => {
        const resolvedMosaic = await helper.resolveMosaicId(
            transactionObj.mosaic,
        );

        const mosaic = new Mosaic(
            new MosaicId(resolvedMosaic.toHex()),
            transactionObj.mosaic.amount,
        );

        const mosaicsFieldObject = await helper.mosaicsFieldObjectBuilder([
            mosaic,
        ]);
    ...
    static accountAddressRestriction = async (transactionObj) => {
        const [addressAdditions, addressDeletions] = await Promise.all([
            Promise.all(
                transactionObj.restrictionAdditions.map((address) => {
                    return helper.resolvedAddress(address);
                }),
            ),
            Promise.all(
                transactionObj.restrictionDeletions.map((address) => {
                    return helper.resolvedAddress(address);
                }),
            ),
        ]);
            ...
            transactionBody: {
                ...
                restrictionType:
                    Constants.AddressRestrictionFlag[
                        transactionObj.restrictionFlags
                    ],
    ...
    static accountMosaicRestriction = async (transactionObj) => {
        ...
        return {
            ...
            transactionBody: {
                ...
                restrictionMosaicAdditions: transactionObj.restrictionAdditions.map(
                    (restriction) => restriction.id.toHex(),
                ),
                restrictionMosaicDeletions: transactionObj.restrictionDeletions.map(
                    (restriction) => restriction.id.toHex(),
                ),
            },
        };
    ...
    static accountOperationRestriction = async (transactionObj) => {
        return {
            ...
            transactionBody: {
                ...
                restrictionOperationAdditions: transactionObj.restrictionAdditions.map(
                    (operation) => operation,
                ),
                restrictionOperationDeletions: transactionObj.restrictionDeletions.map(
                    (operation) => operation,
                ),
            },
        };
    ...
    static mosaicAddressRestriction = async (transactionObj) => {
        ...
        const mosaicAliasName = await helper.getSingleMosaicAliasName(
            resolvedMosaic,
        );
    ...
    static mosaicGlobalRestriction = async (transactionObj) => {
        const referenceMosaicId =
            transactionObj.referenceMosaicId.toHex() === '0000000000000000'
                ? transactionObj.mosaicId
        ...
        return {
            ...
            transactionBody: {
                ...
                newRestrictionType:
                    Constants.MosaicRestrictionType[
                        transactionObj.newRestrictionType
                    ],
    ...
    static namespaceMetadata = async (transactionObj) => {
        const [namespaceName, resolvedAddress] = await Promise.all(
            NamespaceService.getNamespacesNames([
                transactionObj.targetNamespaceId,
            ]),
            helper.resolvedAddress(transactionObj.targetAddress.address),
        );
