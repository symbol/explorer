...
class BlockService {
    ...
    static getMerkleTransactionTree = async (height) => {
        ...
        const leaves = transactions
            .sort(
                (n1, n2) => n1.transactionInfo.index - n2.transactionInfo.index,
            )
    ...
    static getBlockReceiptsInfo = async (height) => {
        ...
        const [blockReceipts, address, mosaic] = await Promise.all([
            ReceiptService.streamerReceipts(searchCriteria),
    ...
    static getBlockInfo = async (height) => {
        ...
        stateHashSubCacheMerkleRoots = stateHashSubCacheMerkleRoots.map(
            (root, index) => {
