...
export default class http {
    static init = async (nodeUrl, marketDataUrl) => {
        ...
        NETWORK_TYPE = await http.createRepositoryFactory
            .getNetworkType()
            .toPromise();
        GENERATION_HASH = await http.createRepositoryFactory
            .getGenerationHash()
            .toPromise();
        NETWORK_PROPERTIES = await http.createRepositoryFactory
            .createNetworkRepository()
            .getNetworkProperties()
            .toPromise();

        const mosaicId = NETWORK_PROPERTIES.chain.currencyMosaicId.replace(
            /0x|'/g,
            '',
        );
        ...
        NATIVE_NAMESPACES =
            (await NamespaceService.getNativeNamespaces()) || [];
    };

    static get networkCurrency() {
        const networkNamespace =
            NETWORK_CURRECY?.mosaicAliasName.toUpperCase() ||
            globalConfig.networkConfig.namespaceName.toUpperCase();
        ...
        return {
            ...
            mosaicId:
                NETWORK_CURRECY?.mosaicId ||
                globalConfig.networkConfig.mosaicId,
            divisibility:
                NETWORK_CURRECY?.divisibility ||
    ...
    static get networkConfig() {
        const convertedTotalChainImportance = +this.networkProperties.chain.totalChainImportance.replace(
            /'/g,
            '',
        );
        const convertedNamespaceGracePeriodDuration = +this.networkProperties.plugins.namespace.namespaceGracePeriodDuration.replace(
            /d/g,
            '',
        );
        const convertedBlockGenerationTargetTime = +this.networkProperties.chain.blockGenerationTargetTime.replace(
            /s/g,
            '',
        );
        ...
        return {
            MosaicRentalSinkAddress: symbol.Address.createFromRawAddress(
                this.networkProperties.plugins.mosaic
                    .mosaicRentalFeeSinkAddress,
            ),
            NamespaceRentalFeeSinkAddress: symbol.Address.createFromRawAddress(
                this.networkProperties.plugins.namespace
    ...
    static get blockPaginationStreamer() {
        return new symbol.BlockPaginationStreamer(
            this.createRepositoryFactory.createBlockRepository(),
        );
    }

    static transactionStatementPaginationStreamer() {
        return symbol.ReceiptPaginationStreamer.transactionStatements(
            this.createRepositoryFactory.createReceiptRepository(),
        );
    }

    static addressResolutionStatementPaginationStreamer() {
        return symbol.ReceiptPaginationStreamer.addressResolutionStatements(
            this.createRepositoryFactory.createReceiptRepository(),
        );
    }

    static mosaicResolutionStatementPaginationStreamer() {
        return symbol.ReceiptPaginationStreamer.mosaicResolutionStatements(
            this.createRepositoryFactory.createReceiptRepository(),
        );
    }

    static get transactionPaginationStreamer() {
        return new symbol.TransactionPaginationStreamer(
            this.createRepositoryFactory.createTransactionRepository(),
        );
    }
}
