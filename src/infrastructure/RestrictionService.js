...
class RestrictionService {
    ...
    static getAccountRestrictions = async (address) => {
        ...
        const formattedAccountRestrictions = accountRestrictions.map(
            (accountRestriction) =>
                this.formatAccountRestriction(accountRestriction),
        );
     ...
     */
    static searchMosaicRestrictions = async (
        ...
        return {
            ...
            data: searchMosaicRestrictions.data.map((mosaicRestriction) => {
                if (mosaicRestriction instanceof MosaicAddressRestriction)
                    return this.formatMosaicAddressRestriction(
                        mosaicRestriction,
                    );

                if (mosaicRestriction instanceof MosaicGlobalRestriction)
                    return this.formatMosaicGlobalRestriction(
                        mosaicRestriction,
                    );
            }),
        };
        ...
        switch (accountRestriction.restrictionFlags) {
            ...
            case AddressRestrictionFlag.BlockOutgoingAddress:
                return {
                    ...
                    restrictionAddressValues: accountRestriction.values.map(
                        (value) => value.address,
                    ),
                };
            ...
            case MosaicRestrictionFlag.BlockMosaic:
                return {
                    ...
                    restrictionMosaicValues: accountRestriction.values.map(
                        (value) => value.id.toHex(),
                    ),
                };
            ...
            case OperationRestrictionFlag.BlockOutgoingTransactionType:
                return {
                    ...
                    restrictionTransactionValues: accountRestriction.values.map(
                        (value) => Constants.TransactionType[value],
                    ),
                };
        }
    };
    ...
    static formatMosaicGlobalRestriction = (mosaicRestriction) => {
        ...
        return {
            ...mosaicRestriction,
            entryType:
                Constants.MosaicRestrictionEntryType[
                    mosaicRestriction.entryType
                ],
            mosaicId: mosaicRestriction.mosaicId.toHex(),
            restrictions: mosaicGlobalRestrictionItem.map((item) => ({
                restrictionKey: item.key,
                restrictionType:
                    Constants.MosaicRestrictionType[item.restrictionType],
                restrictionValue: item.restrictionValue,
                referenceMosaicId:
                    item.referenceMosaicId.toHex() === '0000000000000000'
    ...
    static getMosaicRestrictionList = async (
        ...
        const mosaicRestrictions = await this.searchMosaicRestrictions(
            searchCriteria,
        );
    ...
    static getMosaicAddressRestrictionList = async (pageInfo, address) => {
        ...
        const addressRestrictions = await this.searchMosaicRestrictions(
            searchCriteria,
        );
