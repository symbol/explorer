...
class MosaicService {
    ...
    static searchMosaics = async (mosaicSearchCriteria) => {
        ...
        return {
            ...searchMosaics,
            data: searchMosaics.data.map((mosaic) =>
                this.formatMosaicInfo(mosaic),
            ),
        };
    };
    ...
    static getMosaicInfo = async (hexOrNamespace) => {
        const mosaicId = await helper.hexOrNamespaceToId(
            hexOrNamespace,
            'mosaic',
        );
        ...
        return {
            ...mosaicInfo,
            mosaicAliasName: this.extractMosaicNamespace(
                mosaicInfo,
                mosaicNames,
            ),
        };
    ...
    static getMosaicList = async (pageInfo) => {
        ...
        const mosaicIdsList = mosaicInfos.data.map(
            (mosaicInfo) => new MosaicId(mosaicInfo.mosaicId),
        );

        const mosaicNames = await NamespaceService.getMosaicsNames(
            mosaicIdsList,
        );
    ...
    static getMosaicAmountViewList = async (address) => {
        ...
        const mosaicIdsList = mosaicAmountViewInfos.map(
            (mosaicAmountViewInfo) =>
                new MosaicId(mosaicAmountViewInfo.mosaicId),
        );
        const mosaicNames = await NamespaceService.getMosaicsNames(
            mosaicIdsList,
        );
    ...
    static getMosaicMetadataList = async (pageInfo, filterVaule, mosaicId) => {
        ...
        const mosaicMetadatas = await MetadataService.searchMetadatas(
            searchCriteria,
        );
    ...
    static formatMosaicInfo = (mosaicInfo) => ({
        ...
        duration:
            mosaicInfo.duration.compact() > 0
                ? mosaicInfo.duration.compact()
    ...
    static extractMosaicNamespace = (mosaicInfo, mosaicNames) => {
        let mosaicName = mosaicNames.find(
