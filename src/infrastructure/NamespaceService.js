...
class NamespaceService {
    ...
    static getNamespaceInfo = async (hexOrNamespace) => {
        ...
        formattedNamespaceInfo.expiredInBlock = helper.isNativeNamespace(
        ...
        if (!isExpired) {
            formattedNamespaceInfo.beforeEndHeight = helper.isNativeNamespace(
                namespace.namespaceName,
            )
                ? Constants.Message.INFINITY
        ...
        return {
            ...
            data: namespaceInfos.data.map((namespace) => {
                ...
                return {
                    ...namespace,
                    owneraddress: namespace.ownerAddress,
                    expirationDuration: helper.isNativeNamespace(
                        namespace.namespaceName,
                    )
                        ? Constants.Message.INFINITY
                        : helper.convertTimeFromNowInSec(expiredInSecond),
                    isExpired: isExpired,
                    approximateExpired: helper.isNativeNamespace(
                        namespace.namespaceName,
                    )
                        ? Constants.Message.INFINITY
    ...
    static getNamespaceMetadataList = async (
        ...
        const namespaceMetadatas = await MetadataService.searchMetadatas(
            searchCriteria,
        );
    ...
    static toNamespaces = async (namespaceInfos) => {
        const namespaceIdsList = namespaceInfos.map(
            (namespaceInfo) => namespaceInfo.id,
        );
    ...
    static getNativeNamespaces = async () => {
        ...
        const streamerTransactions = await TransactionService.streamerTransactions(
            searchCriteria,
        );
        const searchNamespaces = streamerTransactions.map((transaction) =>
            TransactionService.formatTransaction(transaction),
        );

        const namespaceids = searchNamespaces.map(
        ...
        const nativeNamespaces = await NamespaceService.toNamespaces(
            namespaceInfos.map((namespaceInfo) => ({
                ...namespaceInfo,
    ...
    static formatNamespace = (namespace) => ({
        ...
        parentName:
            namespace.registrationType !== 0
                ? namespace.name.split('.')[0].toUpperCase()
                : '',
        ...
        return namespaceInfo.levels
            .map((level) => {
                const namespaceName = namespaceNames.find(
                    (name) => name.namespaceId === level.toHex(),
                );
