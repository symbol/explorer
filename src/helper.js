...
class helper {
    static timeSince(interval) {
        ...
        else if (interval.years === 1) return interval.years + ' year';
        else if (interval.days > 1) return interval.days + ' days';
        else if (interval.days === 1) return interval.days + ' day';
        else if (interval.hours > 1) return interval.hours + ' hours';
        else if (interval.hours === 1) return interval.hours + ' hour';
        else if (interval.minutes > 1) return interval.minutes + ' min.';
        // ' minutes'
        else if (interval.minutes === 1) return interval.minutes + ' min.';
        // ' minute'
        else if (interval.seconds !== 1) return interval.seconds + ' sec.';
        // ' seconds'
        else return interval.seconds + ' sec.'; // ' second'
    }
    ...
    static formatSeconds = (second) => {
        ...
        if (m > 0 || h > 0 || d > 0) result = `${m} m ${result}`;

        if (h > 0 || d > 0) result = `${h} h ${result}`;

        if (d > 0) result = `${d} d ${result}`;
    ...
    static isMosaicOrNamespaceId = (str) => str.length === 16;

    static isAccountPublicKey = (str) =>
        str.length === 64 && str.match('^[A-z0-9]+$');

    static isAccountAddress = (str) =>
        str.length === 39 &&
        str.match(
            `[${
                getNetworkTypeAddressFormat[http.networkType]
    ...
    static calculateNamespaceExpiration = (currentHeight, endHeight) => {
        const expired =
            currentHeight >
            endHeight - http.networkConfig.NamespaceGraceDuration;
        const expiredInBlock =
            endHeight -
            http.networkConfig.NamespaceGraceDuration -
            currentHeight;
    ...
    static fetchData = async (
        ...
        try {
            ...
            if (typeof error === 'function') await error(e);
            else {
                console.error(e);
                commit('setError', true);
            }
        }
        if (typeof success === 'function') await success();
    ...
    static hexOrNamespaceToId = async (hexOrNamespace, toId) => {
        ...
        if (isHexadecimal)
            ...
            Id =
                toId === 'mosaic'
                    ? await http.namespace
                          .getLinkedMosaicId(new NamespaceId(hexOrNamespace))
                          .toPromise()
    ...
    static getNetworkCurrencyBalance = (mosaics) => {
        let mosaic = mosaics.find(
            (mosaic) => mosaic.id.toHex() === http.networkCurrency.mosaicId,
        );

        let balance =
            mosaic !== undefined
                ? this.toNetworkCurrency(mosaic.amount)
    ...
    static getLastActivityHeight = (activityBucket) => {
        ...
        let lastActivityHeight =
            activityBucketLength > 0
                ? activityBucket[activityBucketLength - 1].startHeight
                : Constants.Message.UNAVAILABLE;
     ...
     */
    static convertToUTCDate = (networkTimestamp) =>
        moment
            .utc(networkTimestamp * 1000)
            .local()
    ...
    static ImportanceScoreToPercent = (rawScore) => {
        ...
        if (rawScore > 0) percent = rawScore / totalchainimportance;
    ...
    static convertSecondToDate = (second) =>
        moment.utc().add(second, 's').local().format('YYYY-MM-DD HH:mm:ss');
     ...
     * @returns YYYY-MM-DD HH:mm:ss
     */
    static convertDeadlinetoDate = (deadline) =>
        moment.utc(new Date(deadline)).local().format('YYYY-MM-DD HH:mm:ss');
    ...
    static resolvedAddress = async (unResolvedAddress) => {
        ...
        const address = await NamespaceService.getLinkedAddress(
            unResolvedAddress,
        );
    ...
    static resolveMosaicId = async (unresolvedMosaicId) => {
        ...
        const mosaicId = await NamespaceService.getLinkedMosaicId(
            unresolvedMosaicId,
        );
    ...
    static mosaicsFieldObjectBuilder = async (mosaics) => {
        ...
        return resolvedMosaics.map((mosaic) => {
            let divisibility = mosaicInfos.find(
                (info) => info.mosaicId === mosaic.id.toHex(),
            ).divisibility;
    ...
    static isNativeNamespace = (namespaceName) => {
        if (!http.nativeNamespaces) return false;

        const values = http.nativeNamespaces.map((namespace) =>
            namespace.namespaceName.toUpperCase(),
        );
    ...
    static getSingleMosaicAliasName = async (mosaicId) => {
        const getMosaicNames = await NamespaceService.getMosaicsNames([
            mosaicId,
        ]);
        const mosaicAliasName = MosaicService.extractMosaicNamespace(
