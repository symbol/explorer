<template>
    <div class="pagination-wrapper">
        ...
        <ButtonMore v-else-if="!canFetchPrevious" @click="nextPage">{{
            getNameByKey('more')
        }}</ButtonMore>
        <ButtonLess v-else-if="!canFetchNext" @click="previousPage">{{
            getNameByKey('less')
        }}</ButtonLess>
        <div
            v-else
...
export default {
    ...
    methods: {
        nextPage() {
            if (this.nextPageAction) this.$store.dispatch(this.nextPageAction);
            this.$emit('next');
            if (this.goUp) this.goToTop();
        },
