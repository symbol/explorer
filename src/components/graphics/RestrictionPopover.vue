...
export default {
    ...
    computed: {
        titleGlobalRestriction() {
            return (
                `${this.getTranslation('restrictionKey')}: ${
                    this.data.restrictionKey
                }` +
                `${this.getTranslation(
                    'newRestrictionType',
                )}: ${this.getTranslation(this.data.newRestrictionType)}` +
                `${this.getTranslation('newRestrictionValue')}: ${
                    this.data.newRestrictionValue
                }`
            );
        },
        ...
        restrictionTypeText() {
            ...
            switch (restrictionType) {
                case 'mosaicRestrictionType.EQ':
                    return '=';
                case 'mosaicRestrictionType.GE':
                    return '≥';
                case 'mosaicRestrictionType.GT':
                    return '>';
                case 'mosaicRestrictionType.LE':
                    return '≤';
                case 'mosaicRestrictionType.LT':
                    return '<';
                case 'mosaicRestrictionType.NE':
                    return '≠';
                case 'mosaicRestrictionType.NONE':
                    return 'none';
            }
            return '-';
        },
