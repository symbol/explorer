        ...
        addMarkers() {
            ...
            for (const node of this.nodes) {
                const popup =
                    ...
                    node.friendlyName +
                    '"><b>' +
                    this.getNameByKey('friendlyName') +
                    ':</b> ' +
                    this.formatText(node.friendlyName) +
                    '</span><br/><span title="' +
                    node.host +
                    '"><b>' +
                    this.getNameByKey('host') +
                    ':</b> ' +
                    this.formatText(node.host) +
                    '</span><br/><span title="' +
                    node.host +
                    '"><b>' +
                    this.getNameByKey('roles') +
                    ':</b> ' +
                    this.formatText(node.roles) +
                    '</span><br/><span title="' +
                    node.network +
                    '"><b>' +
                    this.getNameByKey('network') +
                    ':</b> ' +
                    this.formatText(node.network) +
                    '</span><br/><span title="' +
                    node.address +
                    '"><b>' +
                    this.getNameByKey('address') +
                    ':</b> ' +
                    this.formatText(node.address) +
                    '</span><br/><span title="' +
                    node.location +
                    '"><b>' +
                    this.getNameByKey('location') +
                    ':</b> ' +
                    this.formatText(node.location) +
                    '</span><br/><span style="' +
                    ...
                    this.button1Style +
                    '" href="' +
                    this.getPageHref('node', node.publicKey) +
                    '">' +
                    this.getNameByKey('nodeDetailTitle') +
                    '</a><a style="' +
                    this.button2Style +
                    '" href="' +
                    this.getPageHref('address', node.address) +
                    '"> ' +
        ...
        formatText(value) {
            return typeof value === 'string'
                ? value.length > 30
                    ? value.slice(0, 15) +
                      '...' +
                      value.slice(value.length - 14, value.length - 1)
                    : value
                : 'n/a';
        },
