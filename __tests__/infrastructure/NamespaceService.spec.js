...
describe('Namespace Service', () => {
    describe('getNamespaceInfo should', () => {
        it('return custom namespace object', async () => {
            const namespace = await NamespaceService.getNamespaceInfo(
                networkConfig.testNamespace.rootNamespace,
            );
            expect(namespace).not.toBeNull();
            expect(namespace.namespaceName).toEqual(
                networkConfig.testNamespace.rootNamespace,
            );
        });
    });
    ...
    describe('getNamespaceList should', () => {
        it('return custom namespace list', async () => {
            const namespaceList = await NamespaceService.getNamespaceList(
                networkConfig.pagination.pageSize,
            );
