...
describe('Transaction Service', () => {
    describe('getTransactionInfo should', () => {
        it('return custom transaction object', async () => {
            const transactionInfo = await TransactionService.getTransactionInfo(
                networkConfig.testTransactions.transferTransactionHash,
            );
            expect(transactionInfo).not.toBeNull();
            expect(transactionInfo.transactionHash).toEqual(
                networkConfig.testTransactions.transferTransactionHash,
            );
        });
    });
